...
public class CalendarGUI extends javax.swing.JFrame {
	...
	// SWAP 1, TEAM 5
	// QUALITY CHANGES
	// I've extracted code into a method here because of code duplication.
	// This method will be called in three different locations.
	// The purpose of this method is to generate a new calendar
	// for the current month if one doesn't exist.
	// This refactor allows for the ability to let the user generate as many months
	// on their schedule as they want, considering we can generate months on demand.
	// Another possible feature would be the ability to look into the future, as this allows
	// us to generate months based on the current year, and we could allow them to specify
	// a year further in the future.
	public void generateMonth(int currentYear, int month) {
	...
	// I've extracted code into a method here because of code duplication.
	// This method will be called in three different locations.
	// This purpose of this method is to add the jobs for a schedule to the
	// table it is supposed to be on.
	// The string for numDate is very slightly different for the different calls,
	// so that variable is passed into the method.
	// It returns the table that the changes were made to.
	// A possible feature that this refactor could enable would be letting the users
	// specify how they want their dates to be displayed on their jobs,
	// as the method takes in the string for numDate. Could also allow for
	// schedule comparisons, as this returns a new DefaultTableModel based on the changes,
	// meaning we could have two different models adding the jobs to see which schedule
	// fits the specified jobs better.
	public DefaultTableModel addJobsToCalendar(DefaultTableModel table, String numDate) {
	...
	public void fillTableForThisMonth() {
		...
		// SWAP 1, TEAM 5
		// QUALITY CHANGES
		// Duplicate code was extracted into new method,
		// so the method is being called here.
		generateMonth(currentYear, this.currentMonth);
		...
		while (this.currentMonth == this.cal.get(Calendar.MONTH) + 1) {
			// SWAP 1, TEAM 5
			// QUALITY CHANGES
			// Duplicate code was extracted from the innards of this while statement.
			// numDate is calculated differently from the other instances of this code
			// so this is still here. The table is set to be the newly modified table
			// that is returned by the addJobsToCalendar method.
			...
			table = addJobsToCalendar(table, numDate);
		}
	...
	public void fillTableMonthAhead() {
		...
		// Duplicate code was extracted into new method,
		// so the method is being called here.
		generateMonth(currentYear, showMonth);
